...
public class MoveScript : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        Vector3 newSped;
        if ((Input.GetKey(KeyCode.LeftShift)) & (canclimb || isClimbing) && !isWallJumping)//TIRMANMAYA G�R��
        {
           ...
           GetComponent<Rigidbody>().useGravity = false;
            GetComponent<Rigidbody>().drag = Climbdrag;//��k��ta d�zeltmeyi unutma
            isJumping = false;//z�plarsa stamina d��ecek
            isClimbing = true;
            newSped = new(0, 0, 0);
            bool duruyomu = true;
            ...
            if (Input.GetKey(KeyCode.Space))//z�pl�yor ve asl�nda ��kt� t�rmanmadan
            {
                PlayerObject.gameObject.GetComponent<Animator>().SetBool("Havadami", true);
                StaminaChange(-ZiplamaStaminaCost);
                isWallJumping = true;
                GetComponent<Rigidbody>().useGravity = true;
                GetComponent<Rigidbody>().drag = originalDrag;//��k��ta d�zeltmeyi unutma
                isJumping = false;//z�plarsa stamina d��ecek
                isClimbing = false;
                speed = MyRigidbdoy.velocity;
        ...
        else if(isWallJumping)
        {
            ...
            if (Input.GetKeyUp(KeyCode.LeftShift))//z�plarken shifti b�rakt���nda tekrar shite basarsa t�rmanmaya devam edecek
            {
                isWallJumping = false;
                canDash = true;
            }
        ...
        if(isClimbing&& (Input.GetKeyUp(KeyCode.LeftShift)||!canclimb))//t�rmanmadan ��k��
        {
            GetComponent<Rigidbody>().useGravity = true;
            GetComponent<Rigidbody>().drag = originalDrag;//��k��ta d�zeltmeyi unutma
